...
interface ImageData { id: string; src: string; alt?: string }
interface GalleryContextType { selectedImage: ImageData | null; setSelectedImage: (image: ImageData | null) => void }
...
export function GalleryImage({ src, alt, id, className }: { src: string; alt?: string; id: string; className?: string }) {
  const context = React.useContext(GalleryContext);
  if (!context) throw new Error("GalleryImage must be within Gallery");
  return (
    <motion.div
      whileHover="hover" whileTap="tap"
      className={cn("relative mb-4 break-inside-avoid cursor-zoom-in rounded-xl overflow-hidden", className)}
      onClick={() => context.setSelectedImage({ id, src, alt })}
    >
      <motion.img
        layoutId={`image-${id}`} src={src} alt={alt || "Gallery Image"}
      ...
      {selectedImage && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <motion.div
            ...
            onDragEnd={(_, info) => { if (Math.abs(info.offset.y) > 100 || Math.abs(info.velocity.y) > 300) setSelectedImage(null); }}
            onClick={() => setSelectedImage(null)}
          >
            <motion.img
              layoutId={`image-${selectedImage.id}`} src={selectedImage.src} alt={selectedImage.alt}
              className="w-auto h-auto max-w-[95vw] max-h-[90vh] rounded-xl shadow-2xl object-contain will-change-transform"
              draggable={false} transition={spring}
            />
          </motion.div>
          <motion.button
